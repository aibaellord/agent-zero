...
(function () {
  ...
  class BaelPerformanceDashboard {
    ...
    bindEvents() {
      // Close
      this.container
        .querySelector("#perf-close")
        .addEventListener("click", () => this.hide());

      // Actions
      this.container
        .querySelector("#perf-refresh")
        .addEventListener("click", () => this.refresh());
      this.container
        .querySelector("#perf-export")
        .addEventListener("click", () => this.exportData());
      this.container
        .querySelector("#perf-reset")
        .addEventListener("click", () => this.resetMetrics());
    ...
    calculatePercentiles() {
      const values = this.metrics.latency.history
        .map((h) => h.value)
        .sort((a, b) => a - b);
    ...
    switchTab(tabId) {
      ...
      this.container.querySelectorAll(".tab-content").forEach((content) => {
        content.style.display =
          content.id === `tab-${tabId}` ? "block" : "none";
      });
    }
    ...
    updateUI() {
      ...
      this.container.querySelector("#lat-min").textContent =
        (this.metrics.latency.min === Infinity
          ? 0
          : Math.round(this.metrics.latency.min)) + "ms";
      ...
      this.container.querySelector("#cache-ratio-value").textContent =
        Math.round(this.metrics.cache.ratio) + "%";
      this.container
        .querySelector(".cache-ring")
        .style.setProperty("--ratio", this.metrics.cache.ratio + "%");
    ...
    async refresh() {
      ...
      if (window.BaelExploit?.cacheExploit) {
        ...
        if (efficiency) {
          ...
          this.metrics.cache.misses =
            efficiency.misses || this.metrics.cache.misses;
        }
      }
