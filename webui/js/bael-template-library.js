...
(function() {
    ...
    class BaelTemplateLibrary {
        ...
        bindEvents() {
            // Close
            this.container.querySelector('#tpl-close').addEventListener('click', () => this.hide());
            this.container.addEventListener('click', (e) => {
                if (e.target === this.container) this.hide();
            });
            
            // Actions
            this.container.querySelector('#tpl-new').addEventListener('click', () => this.createTemplate());
            this.container.querySelector('#tpl-import').addEventListener('click', () => this.importTemplates());
            this.container.querySelector('#tpl-export').addEventListener('click', () => this.exportTemplates());
...
Mood: {{mood}}
Length: {{length}}`,
                    variables: ['genre', 'setting', 'character', 'theme', 'mood', 'length'],
                    tags: ['story', 'fiction']
                },
...
Focus: {{focus_area}}`,
        ...
        saveUserData() {
            try {
                ...
                localStorage.setItem('bael-template-favorites', JSON.stringify(this.favorites));
        ...
        renderDetail() {
            ...
            // Bind action buttons
            detail.querySelector('#tpl-copy')?.addEventListener('click', () => this.copyTemplate());
            detail.querySelector('#tpl-use')?.addEventListener('click', () => this.useTemplate());
        }
        ...
        useTemplate() {
            ...
            // Track usage
            this.usage[this.selectedTemplate.id] = (this.usage[this.selectedTemplate.id] || 0) + 1;
            this.saveUserData();
            
            // Insert into chat input
            const input = document.querySelector('#chat-input, [data-chat-input], textarea[x-model*="input"]');
            if (input) {
                input.value = prompt;
            ...
            // Also emit event
            window.dispatchEvent(new CustomEvent('bael:template-used', {
        ...
        createTemplate() {
            ...
            const promptText = prompt('Template prompt (use {{variable}} for placeholders):');
            ...
            input.onchange = async (e) => {
                ...
                try {
                    ...
                    if (Array.isArray(imported)) {
                        imported.forEach(t => {
                            t.id = 'imported-' + Date.now() + '-' + Math.random().toString(36).substr(2, 5);
                            t.category = 'custom';
                        });
        ...
        exportTemplates() {
            const toExport = this.selectedCategory === 'all' ? 
                this.templates : 
