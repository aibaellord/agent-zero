    ...
    class BaelCommandCenter {
        ...
        registerDefaultCommands() {
            ...
            this.register({
                ...
                category: 'settings',
                shortcut: 'Ctrl+,',
                action: () => window.dispatchEvent(new CustomEvent('bael:open-settings'))
            });
        ...
        addToRecent(command) {
            ...
            this.recentCommands = this.recentCommands.filter(r => r.id !== command.id);
        ...
        render() {
            ...
            if (!this.query) {
                // Show recent first
                const recentIds = this.recentCommands.map(r => r.id);
                const recent = this.filteredCommands.filter(c => recentIds.includes(c.id));
                const others = this.filteredCommands.filter(c => !recentIds.includes(c.id));
                ...
                if (others.length > 0) {
                    ...
                    html += `<div class="command-section">`;
                    html += others.map((cmd, i) => this.renderCommand(cmd, recent.length + i)).join('');
                    html += `</div>`;
                }
            } else {
                html += `<div class="command-section">`;
                html += this.filteredCommands.map((cmd, i) => this.renderCommand(cmd, i)).join('');
                html += `</div>`;
            }
        ...
        saveRecentCommands() {
            try {
                localStorage.setItem('bael-recent-commands', JSON.stringify(this.recentCommands));
            } catch (e) {
