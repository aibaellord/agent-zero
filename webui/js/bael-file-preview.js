...
(function () {
  ...
  class BaelFilePreview {
    ...
    async loadFile(file) {
      ...
      // Update header
      this.container.querySelector(".preview-icon").textContent =
        this.getFileIcon(file.name);
      this.container.querySelector(".preview-filename").textContent = file.name;
      this.container.querySelector(".preview-path").textContent =
      ...
      try {
        ...
        this.container.querySelector(".file-size").textContent =
          this.formatSize(content?.length || 0);
        this.container.querySelector(".file-lines").textContent =
          `${content?.split("\n")?.length || 0} lines`;
        this.container.querySelector(".file-modified").textContent =
    ...
    download() {
      if (!this.currentFile) return;
      window.dispatchEvent(
        new CustomEvent("bael:download-file", {
    ...
    previewFromSelection() {
      const selectedFile = document.querySelector(
        ".file-item.selected, [data-file].selected",
      );
      if (selectedFile) {
        const file = {
