...
(function() {
    ...
    class BaelPromptech {
            ...
            return [
                {
                    ...
                    icon: '📖',
                    category: 'Code',
                    template: 'Please explain the following code in detail, including what each section does:\n\n```{language}\n{code}\n```\n\nProvide:\n1. Overview of functionality\n2. Step-by-step breakdown\n3. Any potential issues or improvements'
                },
                {
                    id: 'debug',
                    name: 'Debug Issue',
                    icon: '🐛',
                    category: 'Code',
                    template: 'I\'m encountering an issue with this code:\n\n```{language}\n{code}\n```\n\nError message: {error}\n\nPlease help me:\n1. Identify the root cause\n2. Explain why it\'s happening\n3. Provide a fix'
                },
                {
                    id: 'optimize',
                    name: 'Optimize Code',
                    icon: '⚡',
                    category: 'Code',
                    template: 'Please optimize the following code for {goal} (e.g., performance, readability, memory usage):\n\n```{language}\n{code}\n```\n\nProvide the optimized version with explanations of changes made.'
                },
                {
                    id: 'test',
                    name: 'Write Tests',
                ...
                },
                {
                    ...
                    icon: '📝',
                    category: 'Writing',
                    template: 'Please summarize the following text in {length} (short/medium/detailed):\n\n{text}\n\nProvide key points and main takeaways.'
                },
                {
                    id: 'rewrite',
                    name: 'Rewrite Text',
                    icon: '✍️',
                    category: 'Writing',
                    template: 'Please rewrite the following text to be more {style} (professional/casual/technical/simple):\n\n{text}'
                },
                {
                    id: 'translate',
                    name: 'Translate',
                    icon: '🌐',
                    category: 'Writing',
                    template: 'Please translate the following text from {from_language} to {to_language}:\n\n{text}\n\nMaintain the original tone and meaning.'
                },
                {
                    id: 'analyze',
                    name: 'Analyze Data',
                ...
                },
                {
                    ...
                    icon: '⚖️',
                    category: 'Analysis',
                    template: 'Please compare the following options:\n\nOption A: {option_a}\nOption B: {option_b}\n\nProvide a detailed comparison including:\n- Pros and cons of each\n- Key differences\n- Recommendation based on {criteria}'
                },
                {
                    id: 'brainstorm',
                    ...
                    icon: '💡',
                    category: 'Creative',
                    template: 'Please brainstorm {count} ideas for {topic}.\n\nContext: {context}\n\nRequirements: {requirements}\n\nProvide creative and diverse suggestions.'
                },
                {
                    id: 'roleplay',
                    name: 'Act As Expert',
        ...
        renderHistory() {
            ...
            return this.history.slice(0, 20).map((item, i) => `
        ...
        updatePreview() {
            ...
            variablesContainer.innerHTML = variables.length > 0 
                ? variables.map(v => `<span class="variable-tag" onclick="BaelPromptech.focusVariable('${v}')">{${v}}</span>`).join('')
            ...
            preview.innerHTML = previewText || 'Enter a template above to see preview...';
        }
        ...
        loadTemplates() {
            try {
                return JSON.parse(localStorage.getItem('bael_promptech_templates') || '[]');
            } catch {
                return [];
            }
        }
        ...
        saveTemplates() {
            localStorage.setItem('bael_promptech_templates', JSON.stringify(this.templates));
        }

        loadHistory() {
            try {
                return JSON.parse(localStorage.getItem('bael_promptech_history') || '[]');
            } catch {
                return [];
            }
        }
        ...
        saveHistory() {
            localStorage.setItem('bael_promptech_history', JSON.stringify(this.history));
        }

        loadVariables() {
            try {
                return JSON.parse(localStorage.getItem('bael_promptech_variables') || '{}');
            } catch {
                return {};
            }
        }
