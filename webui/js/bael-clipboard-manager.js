...
(function () {
  ...
  class BaelClipboardManager {
    ...
    saveToStorage() {
      try {
        localStorage.setItem(
          "bael-clipboard-history",
          JSON.stringify(this.history.slice(0, this.maxHistory)),
        );
        localStorage.setItem(
          "bael-clipboard-pinned",
          JSON.stringify(this.pinnedItems),
        );
      } catch (e) {
    ...
    detectType(text) {
      ...
      // Code detection
      if (
        /^(function|const|let|var|class|import|export|if|for|while|def|async|await)/m.test(
          trimmed,
        ) ||
        /[{}\[\]();]/.test(trimmed) ||
        /^\s*(#|\/\/|\/\*|\*)/m.test(trimmed)
    ...
    setCategory(category) {
      ...
      this.container.querySelectorAll(".clipboard-tab").forEach((tab) => {
        tab.classList.toggle(
          "active",
          tab.textContent.includes(this.getCategoryLabel(category)),
        );
      });
