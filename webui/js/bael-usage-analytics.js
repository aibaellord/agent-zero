...
(function () {
  ...
  class BaelUsageAnalytics {
    ...
    getStats(period = "week") {
      ...
      const sessions = this.analytics.sessions.filter(
        (s) => s.startTime >= cutoff,
      );
      const messages = this.analytics.messages.filter(
        (m) => m.timestamp >= cutoff,
      );
      const commands = this.analytics.commands.filter(
      ...
      return {
        ...
        messagesPerSession:
          sessions.length > 0 ? messages.length / sessions.length : 0,
      };
    }
    ...
    getActivityData(days = 14) {
      ...
      for (let i = days - 1; i >= 0; i--) {
        ...
        const count = this.analytics.messages.filter(
          (m) =>
                        ...
                        <div class="chart-card">
                            ...
                            <div class="features-list">
                                ...
                                `,
                                        ...
                                        .join("")
                                    : '<div style="color: rgba(255,255,255,0.4)">No feature usage tracked yet</div>'
                                }
                            </div>
                        </div>
                    </div>
