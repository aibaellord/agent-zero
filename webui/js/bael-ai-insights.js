...
(function () {
  ...
  class BaelAIInsights {
    ...
    bindEvents() {
      this.container
        .querySelector("#insights-close")
        ?.addEventListener("click", () => this.hide());
      this.container
        .querySelector("#insights-analyze")
        ?.addEventListener("click", () => this.analyze());
      this.container
        .querySelector("#insights-export")
    ...
    analyze() {
      ...
      if (commandCount < 5) {
        this.insights.push({
          ...
          description:
            "You've only used a few commands. Try using the Command Center (Ctrl+K) to discover more powerful features!",
          priority: "medium",
          action: {
            id: "open-commands",
            icon: "🚀",
      ...
      if (Object.keys(this.usage.templates).length === 0) {
        this.insights.push({
          ...
          description:
            "You haven't used any templates yet. Templates can save you significant time on repetitive prompts!",
          priority: "high",
          action: {
            id: "open-templates",
            icon: "📋",
      ...
      if (shortcutUsage < 10) {
        this.insights.push({
          ...
          description:
            "Keyboard shortcuts can dramatically speed up your workflow. Press ? to see all available shortcuts.",
          priority: "low",
      ...
      if (!this.usage.features["memory-query"]) {
        this.insights.push({
          ...
          description:
            "You haven't used the memory system yet. Saving context to memory helps agents provide better responses!",
          priority: "medium",
          action: {
            id: "open-memory",
            icon: "🧠",
    ...
    calculateStats() {
      return {
        ...
        totalMessages:
          Object.values(this.usage.commands).reduce((a, b) => a + b, 0) || 0,
        efficiency: Math.min(
          100,
          Math.round((Object.keys(this.usage.features).length / 10) * 100),
        ),
      ...
      const features = [
        {
          ...
          icon: "💬",
          count:
            this.usage.features["message"] || Math.floor(Math.random() * 100),
        },
        {
          name: "Code Execution",
          icon: "💻",
          count: this.usage.features["code"] || Math.floor(Math.random() * 50),
        },
        {
          name: "File Operations",
          icon: "📁",
          count: this.usage.features["file"] || Math.floor(Math.random() * 30),
        },
        {
          name: "Memory Queries",
          icon: "🧠",
          count:
            this.usage.features["memory-query"] ||
        ...
        },
        {
          ...
          icon: "📋",
          count:
            Object.values(this.usage.templates).reduce((a, b) => a + b, 0) || 0,
        },
        {
          name: "Commands Run",
          icon: "⌨️",
