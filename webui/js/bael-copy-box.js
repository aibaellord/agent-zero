...
(function() {
    ...
    class BaelCopyBox {
            ...
            this.container.innerHTML = `
                <div class="bael-copybox">
                    ...
                    <div class="copybox-content" id="copybox-items">
                        ...
                        ` : items.map((item, i) => this.renderItem(item, i, tab)).join('')}
                    </div>
        ...
        detectType(text) {
            ...
            if (text.includes('function') || text.includes('const ') || text.includes('let ')) return 'code';
        ...
        paste(index) {
            const items = this.currentTab === 'history' ? this.history : this.templates;
        ...
        filter(query) {
            ...
            const filtered = items.filter(item => 
                (item.content || item.text || '').toLowerCase().includes(query.toLowerCase()) ||
            ...
            if (container) {
                container.innerHTML = filtered.length === 0 
                    ...
                    : filtered.map((item, i) => this.renderItem(item, items.indexOf(item), this.currentTab)).join('');
            }
        }
        ...
        showToast(message, type = 'success') {
            ...
            this.toast.style.background = type === 'warning' 
                ? 'rgba(245, 158, 11, 0.9)' 
                : 'rgba(16, 185, 129, 0.9)';
        ...
        saveHistory() {
            localStorage.setItem('bael_copybox_history', JSON.stringify(this.history));
        }

        loadTemplates() {
            try {
                return JSON.parse(localStorage.getItem('bael_copybox_templates') || '[]');
            } catch {
                return [];
            }
        }
        ...
        saveTemplates() {
            localStorage.setItem('bael_copybox_templates', JSON.stringify(this.templates));
        }
