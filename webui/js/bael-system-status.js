...
(function () {
  ...
  class BaelSystemStatus {
    ...
    renderModuleList() {
      ...
      return (
        modules
          ...
          .join("") +
        (modules.length > 20
          ? `<div style="color:#666;">...and ${modules.length - 20} more</div>`
          : "")
      );
    }
    ...
    bindEvents() {
      this.container
        .querySelector("#status-close")
        .addEventListener("click", () => this.hide());
      this.container
        .querySelector("#status-refresh")
        .addEventListener("click", () => this.refresh());
      this.container
        .querySelector("#status-toggle-refresh")
        .addEventListener("click", () => this.toggleAutoRefresh());
    ...
    async refresh() {
      try {
        ...
        if (performance.memory) {
          ...
          this.status.memory.percent = Math.round(
            (this.status.memory.used / this.status.memory.total) * 100,
          );
        }
