    ...
    class BaelReachBox {
        ...
        buildSources() {
            ...
            this.sources = [
                ...
                // Quick actions
                { icon: '🆕', title: 'New Chat', desc: 'Start a new conversation', group: 'Actions', action: () => this.executeCommand('newChat') },
                { icon: '💾', title: 'Save Session', desc: 'Save current session', group: 'Actions', action: () => this.executeCommand('saveSession') },
                { icon: '📤', title: 'Export Chat', desc: 'Export chat history', group: 'Actions', action: () => this.executeCommand('exportChat') },
                { icon: '🔄', title: 'Refresh', desc: 'Refresh the page', group: 'Actions', action: () => location.reload() },
                { icon: '🌙', title: 'Toggle Dark Mode', desc: 'Switch theme', group: 'Actions', action: () => this.toggleTheme() },
                
                // Settings
                { icon: '⚙️', title: 'Settings', desc: 'Open settings panel', group: 'Settings', action: () => this.openSettings() },
                { icon: '🔑', title: 'API Keys', desc: 'Manage API keys', group: 'Settings', action: () => this.openSettings('api') },
                { icon: '📊', title: 'Performance', desc: 'View performance stats', group: 'Settings', shortcut: 'Ctrl+Shift+P' },
                
                // Navigation
                { icon: '📁', title: 'Files', desc: 'Open file browser', group: 'Navigation', action: () => this.navigate('files') },
                { icon: '💡', title: 'Knowledge', desc: 'View knowledge base', group: 'Navigation', action: () => this.navigate('knowledge') },
                { icon: '📝', title: 'Prompts', desc: 'Browse prompts', group: 'Navigation', action: () => this.navigate('prompts') },
                
                // Help
                { icon: '❓', title: 'Help', desc: 'View documentation', group: 'Help', action: () => this.openHelp() },
                { icon: '⌨️', title: 'Keyboard Shortcuts', desc: 'View all shortcuts', group: 'Help', shortcut: 'Ctrl+Shift+K' }
            ];
        }
        ...
        getBaelModules() {
            ...
            Object.keys(window).filter(k => k.startsWith('Bael')).forEach(name => {
                modules.push({
                    name,
        ...
        search(query) {
            ...
            const filtered = this.sources.filter(item => 
        ...
        showDefaultResults() {
            ...
            const recent = this.sources.filter(s => s.group === 'Recent').slice(0, 3);
            const modules = this.sources.filter(s => s.group === 'Modules').slice(0, 5);
            const actions = this.sources.filter(s => s.group === 'Actions').slice(0, 4);
        ...
        updateSelection() {
            this.resultsContainer.querySelectorAll('.result-item').forEach((el, i) => {
        ...
        addToRecent(item) {
            ...
            this.recentActions = [recentItem, ...this.recentActions.filter(r => r.title !== item.title)].slice(0, 5);
            this.saveRecent();
        }
        ...
        saveRecent() {
            localStorage.setItem('bael_reachbox_recent', JSON.stringify(this.recentActions));
        }
