    ...
    class BaelUnifiedSearch {
        ...
        bindEvents() {
            ...
            this.container.querySelectorAll('.scope-tab').forEach(tab => {
                tab.addEventListener('click', () => {
                    ...
                    this.container.querySelectorAll('.scope-tab').forEach(t => t.classList.remove('active'));
        ...
        searchCommands(q) {
            const commands = [
                ...
                { name: 'Settings', action: 'settings', icon: '⚙️', shortcut: 'Ctrl+,' },
                { name: 'Memory Dashboard', action: 'memoryDashboard', icon: '🧠', shortcut: 'Ctrl+Shift+M' },
                { name: 'Agent Console', action: 'agentConsole', icon: '📊', shortcut: 'Ctrl+Shift+A' },
                { name: 'Heisenberg Control', action: 'heisenberg', icon: '⚛️', shortcut: 'Ctrl+Shift+H' },
                { name: 'Performance Dashboard', action: 'perfDashboard', icon: '📈', shortcut: 'Ctrl+Shift+P' },
                { name: 'Command Center', action: 'commandCenter', icon: '🎯', shortcut: 'Ctrl+K' },
                { name: 'Template Library', action: 'templates', icon: '📋', shortcut: 'Ctrl+Shift+T' },
                { name: 'File Browser', action: 'fileBrowser', icon: '📁', shortcut: '' },
                { name: 'Scheduler', action: 'scheduler', icon: '📅', shortcut: '' },
                { name: 'Toggle Dark Mode', action: 'darkMode', icon: '🌙', shortcut: '' },
                { name: 'Restart Framework', action: 'restart', icon: '🔄', shortcut: '' }
            ];
        ...
        updateSelection() {
            this.container.querySelectorAll('.search-result-item').forEach((item, i) => {
        ...
        executeSelected() {
            ...
            switch (result.action) {
                ...
                case 'openFile':
                    ...
                    break;
                case 'openChat':
                    window.dispatchEvent(new CustomEvent('bael:open-chat', {
                        detail: result.data
                    }));
                    break;
            }
        ...
        addToRecent(query) {
            ...
            // Save
            localStorage.setItem('bael-recent-searches', JSON.stringify(this.recentSearches));
        }
