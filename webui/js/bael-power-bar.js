...
(function() {
    ...
    class BaelPowerBar {
        ...
        getTemplate() {
            const actions = [
                { icon: '🚀', action: 'launcher', tooltip: 'Quick Launcher', shortcut: '⌃⌃' },
                { icon: '🔍', action: 'search', tooltip: 'Universal Search', shortcut: '⌃/' },
                { icon: '🎯', action: 'command', tooltip: 'Command Center', shortcut: '⌃K' },
                { type: 'separator' },
                { icon: '💬', action: 'new-chat', tooltip: 'New Chat', shortcut: '⌃N' },
                { icon: '📋', action: 'templates', tooltip: 'Templates', shortcut: '⌃⇧T' },
                { icon: '⭐', action: 'favorites', tooltip: 'Favorites', shortcut: '⌃⇧B' },
                { type: 'separator' },
                { icon: '📊', action: 'performance', tooltip: 'Performance', shortcut: '⌃⇧P' },
                { icon: '🧠', action: 'memory', tooltip: 'Memory', shortcut: '⌃⇧M' },
                { icon: '📡', action: 'status', tooltip: 'System Status', shortcut: '⌃⇧S' },
                { type: 'separator' },
        ...
        onDrag(e) {
            ...
            const x = Math.max(0, Math.min(e.clientX - this.dragOffset.x, window.innerWidth - 60));
            const y = Math.max(0, Math.min(e.clientY - this.dragOffset.y, window.innerHeight - 400));
        ...
        savePosition() {
            localStorage.setItem('bael_power_bar_position', JSON.stringify(this.position));
        }
