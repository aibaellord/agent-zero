...
(function() {
    ...
    class BaelHeisenbergControl {
        ...
        bindEvents() {
            // Close
            this.container.querySelector('#hei-close').addEventListener('click', () => this.hide());
            
            // Actions
            this.container.querySelector('#hei-pause').addEventListener('click', () => this.togglePause());
            this.container.querySelector('#hei-refresh').addEventListener('click', () => this.refresh());
            this.container.querySelector('#hei-export').addEventListener('click', () => this.exportData());
        ...
        updateUI() {
            ...
            this.container.querySelector('#metric-confidence').textContent = confidence + '%';
            this.container.querySelector('#metric-patterns').textContent = patterns;
            this.container.querySelector('#metric-connections').textContent = connections;
            ...
            const contextPercent = contextMax > 0 ? (contextUsed / contextMax * 100) : 0;
            
            this.container.querySelector('#context-fill').style.width = contextPercent + '%';
        ...
        updateReasoningList(insights) {
            ...
            list.innerHTML = insights.slice(0, 10).map(insight => `
        ...
        exportData() {
            const data = {
                ...
                metrics: {
                    reasoning: this.container.querySelector('#metric-reasoning').textContent,
                    confidence: this.container.querySelector('#metric-confidence').textContent,
                    patterns: this.container.querySelector('#metric-patterns').textContent,
                    connections: this.container.querySelector('#metric-connections').textContent
                }
            };
