...
(function() {
    ...
    class BaelVoiceCommander {
        ...
        registerDefaultCommands() {
            // Navigation commands
            this.registerCommand(['new chat', 'start new chat', 'new conversation'], () => {
                this.executeAction('new-chat');
            });
            
            this.registerCommand(['open settings', 'settings', 'go to settings'], () => {
            ...
            this.registerCommand(['open launcher', 'launcher', 'quick launch'], () => {
            ...
            this.registerCommand(['type'], (text) => {
                this.insertText(text);
            }, { prefix: true });
            
            // Ask commands (prepend with "ask")
            this.registerCommand(['ask', 'question'], (text) => {
                this.insertText(text);
        ...
        executeAction(action) {
            switch (action) {
                case 'new-chat':
                    const newChatBtn = document.querySelector('[x-on\\:click*="newChat"], .new-chat-btn');
                    if (newChatBtn) newChatBtn.click();
                    break;
                    
                case 'settings':
                    const settingsBtn = document.querySelector('[x-on\\:click*="settings"], .settings-btn, [href*="settings"]');
                    if (settingsBtn) settingsBtn.click();
                    break;
                    
                case 'send':
                    const sendBtn = document.querySelector('[type="submit"], [x-on\\:click*="send"]');
                    if (sendBtn) sendBtn.click();
                    break;
                    
                case 'clear':
                    const input = document.querySelector('#msg-input, textarea[name="message"]');
                    if (input) {
                        input.value = '';
                ...
                case 'pause':
                    const pauseBtn = document.querySelector('[x-on\\:click*="pause"], .pause-btn');
                    if (pauseBtn) pauseBtn.click();
                    break;
                    
                case 'resume':
                    const resumeBtn = document.querySelector('[x-on\\:click*="resume"], .resume-btn');
        ...
        updateIndicator() {
            if (this.listening) {
                ...
                this.indicator.querySelector('.voice-status').textContent = 'Listening...';
            } else {
                this.indicator.classList.remove('listening');
                this.indicator.querySelector('.voice-status').textContent = 'Ready to listen';
            }
        }

        updateTranscript(text) {
            this.transcript = text;
            this.indicator.querySelector('.voice-transcript').textContent = text || 'Say a command...';
        }
