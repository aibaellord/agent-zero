    ...
    class BaelSmartSuggest {
        constructor() {
            ...
            this.commandPatterns = [
                { prefix: '/', commands: ['help', 'clear', 'new', 'save', 'export', 'settings', 'agents', 'memory', 'search'] },
                { prefix: '@', commands: ['agent', 'file', 'url', 'code', 'image'] },
                { prefix: '#', commands: ['important', 'todo', 'question', 'bug', 'feature', 'idea'] }
            ];
            
            // Prompt templates
            this.promptTemplates = [
                { label: 'Explain', template: 'Explain {concept} in simple terms', icon: '📖' },
                { label: 'Debug', template: 'Help me debug this code: {code}', icon: '🐛' },
                { label: 'Write Code', template: 'Write a {language} function that {description}', icon: '💻' },
                { label: 'Summarize', template: 'Summarize the following: {content}', icon: '📝' },
                { label: 'Compare', template: 'Compare {item1} vs {item2}', icon: '⚖️' },
                { label: 'Improve', template: 'Improve this code for {criteria}: {code}', icon: '✨' },
                { label: 'Create Test', template: 'Write unit tests for: {code}', icon: '🧪' },
                { label: 'Document', template: 'Add documentation/comments to: {code}', icon: '📚' },
                { label: 'Refactor', template: 'Refactor this code: {code}', icon: '🔄' },
                { label: 'Translate', template: 'Translate to {language}: {text}', icon: '🌐' }
            ];
        }
        ...
        saveHistory() {
            try {
                localStorage.setItem('bael-suggest-history', JSON.stringify(this.history.slice(0, 50)));
            } catch (e) {}
        }
        ...
        onInput(e) {
            ...
            for (const pattern of this.commandPatterns) {
                if (value.startsWith(pattern.prefix)) {
                    ...
                    this.showCommands(pattern.prefix, pattern.commands.filter(c => c.includes(query)));
                    return;
                }
            }
            ...
            const templateMatches = this.templates.filter(t =>
        ...
        render(mode) {
            ...
            if (mode === 'templates') {
                ...
                const historyItems = this.suggestions.filter(s => s.type === 'history');
                const templateItems = this.suggestions.filter(s => s.type === 'template');
        ...
        renderItem(suggestion, index) {
            const iconClass = suggestion.type === 'command' ? 'command' : 
